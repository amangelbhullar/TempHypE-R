"""
TempHypE-R V4 Ablation Study on WIKI
"""
        ...
        for ts_id, quads in train_groups:
            ...
            for start in range(0, len(quads), 64):
                ...
                if no_contrast:
                    ...
                    closs = model.compute_contrastive(h,r,t,tau,snapshots,delta_t)
...
def run_variant(name, data, train_groups, snapshots, ts_to_real,
    ...
    return train_eval(model, train_groups, data, snapshots, ts_to_real,
                      optimizer, scheduler, **train_kwargs)

def main():
    print("Loading WIKI...", flush=True)
    ...
    train_groups = sorted(defaultdict(list, {q[3]: [] for q in data.train}).items())
    ...
    HDR = (f"{'Variant':<24} | {'MRR':>6} | {'H@1':>6} | "
           f"{'H@3':>6} | {'H@10':>6} | {'vs Full':>8}")
    SEP = '-' * 65
    results = []

    print(f"\nV4 Ablation — WIKI ({EPOCHS} epochs, patience={PATIENCE})\n")

    for name, model_kw, train_kw in variants:
        best_valid, tm = run_variant(name, data, train_groups,
        ...
        for (n, bv, m) in results:
            ...
            print(f"{n:<24} | {m['MRR']:>6.4f} | {m['Hits@1']:>6.4f} | "
                  f"{m['Hits@3']:>6.4f} | {m['Hits@10']:>6.4f} | {sign}{diff:.4f}")

    print(f"\n{'='*65}")
    print(f"FINAL V4 ABLATION — WIKI")
    print(f"{HDR}\n{SEP}")
    full_mrr = results[0][2]['MRR']
